      ...
      <div className="flex flex-col gap-6">
        ...
        <div style={{ background: '#f8fafc', padding: '1.25rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <div className="flex justify-between items-center" style={{ marginBottom: '0.5rem' }}>
            <label className="input-label" style={{ margin: 0, fontWeight: 700, color: '#0f172a' }}>
          ...
          />
          <div className="flex justify-between text-xs" style={{ color: '#64748b', marginTop: '0.25rem' }}>
                ...
                >
                  <div>
                    <div className="flex justify-between items-start" style={{ marginBottom: '0.5rem' }}>
                      <div>
                        <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '1rem' }}>{plan.bank}</div>
                        <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{plan.cardType}</div>
                      </div>
                    ...
                    <div style={{ margin: '0.75rem 0' }}>
                      <div style={{ fontSize: '1.75rem', fontWeight: 900, color: '#166534' }}>
                        ...
                        <span style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 500 }}> / mo</span>
                      </div>
                ...
                <tbody>
                  {(emiData?.creditCardPlans || []).map((p, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', background: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                      <td style={{ padding: '0.75rem 1rem', fontWeight: 700, color: '#0f172a' }}>{p.bank}</td>
                      ...
                      <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{p.annualRate}% p.a.</td>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {(emiData?.debitCardPlans || []).map((p, idx) => (
                ...
                >
                  <div>
                    <div style={{ fontWeight: 700, color: '#0f172a', fontSize: '0.95rem' }}>{p.bank}</div>
                    <div style={{ fontSize: '0.75rem', color: '#64748b' }}>Tenure: {p.tenureMonths} Months @ {p.annualRate}%</div>
                ...
                >
                  <div>
                    <div className="flex justify-between items-start" style={{ marginBottom: '0.35rem' }}>
                      <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '1rem' }}>{p.partner}</div>
                      {p.badge && (
                    ...
                    <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{p.type} • {p.tenureMonths} Months ({p.annualRate}% p.a.)</div>

                    <div style={{ margin: '0.75rem 0' }}>
                      <div style={{ fontSize: '1.6rem', fontWeight: 900, color: '#166534' }}>
                        {formatINR(p.calculation.monthlyEMI)}
                        <span style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 500 }}> / month</span>
                      </div>
                    </div>
                  </div>

                  <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '0.65rem', fontSize: '0.75rem', color: '#334155', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div style={{ background: '#f8fafc', padding: '1rem', borderRadius: '10px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
          <div className="flex items-center gap-2" style={{ fontSize: '0.8rem', color: '#334155', fontWeight: 600 }}>
            <Shield size={16} color="#166534" />
            <span>Select EMI option at Razorpay Checkout screen during order placement</span>
          </div>
          <div className="flex gap-1.5">
            <span className="badge" style={{ background: '#ffffff', border: '1px solid #cbd5e1', color: '#0f172a', fontSize: '0.7rem' }}>HDFC</span>
            <span className="badge" style={{ background: '#ffffff', border: '1px solid #cbd5e1', color: '#0f172a', fontSize: '0.7rem' }}>SBI</span>
            <span className="badge" style={{ background: '#ffffff', border: '1px solid #cbd5e1', color: '#0f172a', fontSize: '0.7rem' }}>ICICI</span>
            <span className="badge" style={{ background: '#ffffff', border: '1px solid #cbd5e1', color: '#0f172a', fontSize: '0.7rem' }}>Axis</span>
            <span className="badge" style={{ background: '#ffffff', border: '1px solid #cbd5e1', color: '#0f172a', fontSize: '0.7rem' }}>Bajaj Finserv</span>
          </div>
        </div>
      </div>
    </Modal>
